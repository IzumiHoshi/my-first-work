...
BEGIN_MESSAGE_MAP(CChnTranslateDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_BUTTON_GBK, &CChnTranslateDlg::OnBnClickedButtonGbk)
END_MESSAGE_MAP()
...
void CChnTranslateDlg::OnBnClickedButtonUtf8()
{
	...
	{
		TRACE("a%d\n", m_csInput.GetAt(i));
		DWORD n = m_csInput.GetAt(i);
		PublicUtil::ConvertSingleUnicodeToUtf8(n);
		m_csOutput.AppendFormat(_T("%X "), n);
	}

	m_csOutput = m_csOutput.Trim();

	UpdateData(FALSE);
}


void CChnTranslateDlg::OnBnClickedButtonGbk()
{
	UpdateData(TRUE);
	int nLenth = m_csInput.GetLength();
	m_csOutput.Empty();
	...
	for (int i = 0; i < nLenth; i++)
	{
		...
		DWORD n = m_csInput.GetAt(i);
		PublicUtil::ConvertSingleUnicodeToUtf8(n);
		PublicUtil::ConvertSingleUtf8ToUnicode(n);
		m_csOutput.AppendFormat(_T("%X "), n);
	}
